...
class AdminController extends Controller
{
    ...
    protected function _initialize()
    {
        // $where = array(
            // 'is_linqu' => 1,
            // 'pay_state' =>3,
            // 'dj_time'=>array('gt',time())
        // );
        // $res = M('trans')->field('pay_nums,payin_id,dj_time')->where($where)->select();
        // foreach ($res as $key => $value) {
            // $lixi = $value['pay_nums']*$value['dj_time']/60/60/24*0.008;

            //给用户加余额
            // $paynums = $value['pay_nums']+$lixi;
            // $datapay['cangku_num'] = $paynums;
            // $res_pay = M('store')->where(array('uid'=>$res['payin_id']))->save($datapay);

...
class ConfigController extends AdminController
{
  ...
  public function set_coupons(){
    ...
    $need = I('need');
    $dq_time = I('dq_time');
    M('config')->where(array('name'=>'coupons_need'))->setField('value',$need);
    M('config')->where(array('name'=>'coupons_jian'))->setField('value',$jian);
    M('config')->where(array('name'=>'coupons_time'))->setField('value',$dq_time);
    $this->success('保存成功');
  }
  //转盘设置
  public function set_turntable(){
    $data = I();
    $rules = I('rules');
    $where['name'] = 'turntable_rules';
    M('config')->where($where)->setField('value',$rules);
    for ($i=1; $i < 11; $i++) { 
      $name = 'name'.$i;
      $gl = 'gl'.$i;
      M('turntable')->where("id = $i")->setField('name',$data[$name]);
      M('turntable')->where("id = $i")->setField('gl',$data[$gl]);
    }
    $imgname = $_FILES['picture'];
    if($imgname){
      $info = $this->upload();
    }
    foreach ($info as $k => $v) {
      $url[$k] = '/Uploads/'.$v['savepath'].$v['savename'];
      $id = $k+1;
      M('turntable')->where("id = $id")->setField('img',$url[$k]);
    }
    echo "<meta http-equiv='Content-Type'' content='text/html; charset=utf-8'>";
    echo "<script>alert('保存成功');location.href='".U('config/group2')."';</script>";
  }
 ...
 public function group1($group = 4)
    ...
    public function group2($group = 1)
    {
        ...
        $dk_time = D('config')->where('name="dk_time"')->getField('value');
        $djkg = D('config')->where('name="djkg"')->getField('value');
        $djmc = D('config')->where('name="djmc"')->getField('value');
        ...
        $interest = D('config')->where('name="interest"')->getField('value');
        $interest_maxtime = D('config')->where('name="interest_maxtime"')->getField('value');
        $max_paidan = D('config')->where('name="max_paidan"')->getField('value');
        $max_purchase = D('config')->where('name="max_purchase"')->getField('value');
		...
		//hou
		$usernums = M('coindets')->where(array('id'=>3))->getField('coin_price');
		$faxing1 = M('coindets')->where(array('id'=>1))->getField('coin_price');
		$faxing2 = M('coindets')->where(array('id'=>2))->getField('coin_price');
		$usdt = M('coindets')->where(array('id'=>4))->getField('coin_price');
		$usdtsx = M('coindets')->where(array('id'=>5))->getField('coin_price');

    $turntable_rules = M('config')->where(array('name'=>'turntable_rules'))->getField('value');
    $turntable = M('turntable')->select();

    $this->assign('turntable_rules',$turntable_rules);
        ...
        $this->assign([
			...
			'faxing2'=>$faxing2,
			'faxing1'=>$faxing1,
			'usernums'=>(int)$usernums,
            'dk_time'=>$dk_time,
            'djkg'=>$djkg,
            'djmc'=>$djmc,
            'interest' => $interest,
            'interest_maxtime' => $interest_maxtime,
            'max_paidan' => $max_paidan,
            'max_purchase' => $max_purchase,
            'sellcentr_start' => $sellcentr_start,
            'sellcentr_end' => $sellcentr_end,
        ]);
        $this->assign('b_drills',$b_drills)->assign('b_num',$b_num)->assign('g_drills',$g_drills)->assign('money',$money)->assign('service_qq',$service_qq)->assign('service_wx',$service_wx)->assign('g_num',$g_num)->assign('false_user',$false_user)->assign('add_user',$add_user)->assign('min_balance',$min_balance);
...
/**
 ...
 /**
    ...
    public function groupSave2()
    {
        ...
        $dk_time = I('post.dk_time');
        $djkg = I('post.djkg');
        $djmc = I('post.djmc');
		$service_qq = I('service_qq');
		$service_wx = I('service_wx');
        $interest = I('post.interest');
        $interest_maxtime = I('post.interest_maxtime');
        $max_paidan = I('post.max_paidan');
        $max_purchase = I('post.max_purchase');
        $sellcentr_start = I('post.sellcentr_start');
        $sellcentr_end = I('post.sellcentr_end');
		$money = I('money');
        D('Config')->where("name='money'")->setField('value',$money);
        ...
        D('config')->where('name="dk_time"')->setField('value',$dk_time);
        D('config')->where('name="djkg"')->setField('value',$djkg);
        D('config')->where('name="djmc"')->setField('value',$djmc);
		D('Config')->where("name='service_qq'")->setField('value',$service_qq);
		D('Config')->where("name='service_wx'")->setField('value',$service_wx);
        D('config')->where('name="interest"')->setField('value',$interest);
        D('config')->where('name="interest_maxtime"')->setField('value',$interest_maxtime);
        D('config')->where('name="max_paidan"')->setField('value',$max_paidan);
        D('config')->where('name="max_purchase"')->setField('value',$max_purchase);
    ...
    {
      $false_user['coin_price']=I('post.false_user');
	  D('coindets')->where('id=3')->save($false_user);
	  
	  $usdtsx['coin_price']=I('post.usdtsx');
	  D('coindets')->where('id=5')->save($usdtsx);
	  
	  $usdt['coin_price']=I('post.usdt');
      D('coindets')->where('id=4')->save($usdt);
	  
	  $faxing2['coin_price']=I('post.faxing2');
      D('coindets')->where('id=2')->save($faxing2);
	  
	  $faxing1['coin_price']=I('post.faxing1');
      D('coindets')->where('id=1')->save($faxing1);
    ...
    public function faxing()
    {
      ...
      $data['coin_price'] = $false_user;
	  $data2['coin_price'] = $add_user;
      D('coindets')->where('id=2')->save($data);
      D('coindets')->where('id=1')->save($data2);
      $this->success('保存成功！');
    }

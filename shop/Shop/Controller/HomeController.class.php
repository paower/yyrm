...
{
	//优惠券
	public function coupons($id = 0){
		if($_POST){
			$userid = session('userid');
			$overtime = M('config')->where(array('name'=>'coupons_time'))->getField('value');
			$time = $overtime*86400;
			switch ($id) {
				case '0':
					$map['uid'] = $userid;
					$map['status'] = 1;
					$info = M('coupons')->where($map)->select();
					foreach ($info as $k => $v) {
						$id = $v['id'];
						$pid = $v['pid'];
						$time = M('coupons_info')->where("id = $pid")->getField('day')*86400;
						$jstime = $v['have_time']+$time;
						$jstime = date("Y-m-d H:i:s",$jstime);
						$res .= "<div class='weui-cell'><div class='weui-cell__hd' style='position: relative;margin-right: 10px;'><img src='/Public/Public/images/coupons.jpg' style='width: 50px;display: block'></div><div class='weui-cell__bd'><p>优惠券一</p><p style='font-size: 13px;color: red;'>满".$v['need']."减".$v['jian']."元</p><p style='font-size: 13px;color: #888888;'>到期时间：".$jstime."</p></div><div class='weui-cell__right'><p>未使用</p></div></div>";
					}
					if(!$info){
						ajaxReturn('');
					}else{
				...
				case '1':
					...
					foreach ($info as $k => $v) {
						...
						$jstime = $v['have_time']+$time;
						$jstime = date("Y-m-d H:i:s",$jstime);
						$res .= "<div class='weui-cell'><div class='weui-cell__hd' style='position: relative;margin-right: 10px;'><img src='/Public/Public/images/coupons.jpg' style='width: 50px;display: block'></div><div class='weui-cell__bd'><p>优惠券一</p><p style='font-size: 13px;color: red;'>满".$v['need']."减".$v['jian']."元</p><p style='font-size: 13px;color: #888888;'>到期时间：".$jstime."</p></div><div class='weui-cell__right'><p>已使用</p></div></div>";
					}
					if(!$info){
						ajaxReturn('');
					}else{
				...
				case '2':
					...
					foreach ($info as $k => $v) {
						...
						$jstime = $v['have_time']+$time;
						$jstime = date("Y-m-d H:i:s",$jstime);
						$res .= "<div class='weui-cell'><div class='weui-cell__hd' style='position: relative;margin-right: 10px;'><img src='/Public/Public/images/coupons.jpg' style='width: 50px;display: block'></div><div class='weui-cell__bd'><p>优惠券一</p><p style='font-size: 13px;color: red;'>满".$v['need']."减".$v['jian']."元</p><p style='font-size: 13px;color: #888888;'>到期时间：".$jstime."</p></div><div class='weui-cell__right'><p>已到期</p></div></div>";
					}
					if(!$info){
						ajaxReturn('');
					}else{
...
/* var_dump($photo);die;*/
      if(empty($data['username'])){
            echo '<script>alert("请输入充值YKSC账号"); window.history.back(-1); </script>';
...
public function cation()
	...
	public function index()
	{
		//检测优惠券是否过期
		$uid = session("userid");
		$num = M('coupons')->where("uid = $uid")->count();
		if($num>0){
			$coupons = M('coupons')->where("uid = $uid")->select();
			foreach ($coupons as $k => $v) {
				$id = $v['id'];
				$pid = $v['pid'];
				$day = M('coupons_info')->where("id = $pid")->getField('day');
				$overtime =$v['have_time']+$day*86400;
				if($overtime<=time()){
					M('coupons')->where("id = $id")->setField('status',3);
				}
			}
		}
		//查出首页的7个菜单 第8个菜单是分类
...
/*var_dump($filter);die;*/
	...
	public function order()
	{
		...
		$map['need'] = array('elt',$allprice);
		$map['status'] = 1;
		$coupons = M('coupons')->where($map)->select();
		// dump($coupons);die;

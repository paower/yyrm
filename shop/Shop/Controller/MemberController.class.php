...

{
	...
	public function turntablelist(){
		$uid = session('userid');
		$info = M('turntable_details')->where("uid = $uid")->order('time DESC')->select();
		foreach ($info as $k => $v) {
			$gid = $v['gid'];
			$name = M('turntable')->where("id = $gid")->getField('name');
			$info[$k]['name'] = $name;
		}
	...
	//转盘概率
	public function turntable_gl(){
		$uid = session("userid");
		$num = M('user')->where("userid = $uid")->getField('turntable_num');
		if($num == 0){
			ajaxReturn('没有抽奖机会',0);
		}
		$info = M('turntable')->select();
		foreach ($info as $k => $v) {
			$gl = $v['gl'];
			$id = $v['id'];
			for ($i=0; $i < $gl; $i++) { 
		...
		$data['gid'] = $res;
		$data['time'] = time();
		M('turntable_details')->add($data);
		M('user')->where("userid = $uid")->setField('turntable_num',0);
		M('turntable')->where("id = $res")->setDec('gl',1);
		switch ($res) {
			case '1':
				$name = M('turntable')->where("id = 1")->getField('name');
				ajaxReturn(3762,1,$name);
				break;
			case '2':
				$name = M('turntable')->where("id = 2")->getField('name');
				ajaxReturn(3726,1,$name);
				break;
			case '3':
				$name = M('turntable')->where("id = 3")->getField('name');
				ajaxReturn(3690,1,$name);
				break;
			case '4':
				$name = M('turntable')->where("id = 4")->getField('name');
				ajaxReturn(3654,1,$name);
				break;
			case '5':
				$name = M('turntable')->where("id = 5")->getField('name');
				ajaxReturn(3618,1,$name);
				break;
			case '6':
				$name = M('turntable')->where("id = 6")->getField('name');
				ajaxReturn(3582,1,$name);
				break;
			case '7':
				$name = M('turntable')->where("id = 7")->getField('name');
				ajaxReturn(3546,1,$name);
				break;
			case '8':
				$name = M('turntable')->where("id = 8")->getField('name');
				ajaxReturn(3510,1,$name);
				break;
			case '9':
				$name = M('turntable')->where("id = 9")->getField('name');
				ajaxReturn(3474,1,$name);
				break;
			case '10':
				$name = M('turntable')->where("id = 10")->getField('name');
				ajaxReturn(3438,1,$name);
				break;
		}
	}
	...
	public function turntable(){
		...
		$uid = session("userid");
		$num = M('user')->where("userid = $uid")->getField('turntable_num');
		$rules = M('config')->where(array('name'=>'turntable_rules'))->getField('value');

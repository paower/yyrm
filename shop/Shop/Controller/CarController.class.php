...
class CarController extends CommonController
{
	...
	public function addOrder()
	{
        ...
        $coupons = I('coupons');
        $coupons_type = M('coupons')->where("id = $coupons")->getField('type');
        $coupons_pid = M('coupons')->where("id = $coupons")->getField('pid');
        if($coupons_type==3){
        	$product_cate = M('coupons_info')->where("id = $coupons_pid")->getField('product_cate');
        	$product_cate = explode(',',$product_cate);
        }
		#++生成大订单++
        ...
        foreach ($carList as $k => $v) {
            ...
            $where = array();
            $where['id']=$v['pid'];
            $com_info=$commodity->where($where)->field('id,shangjia,price,name,type_id')->find();
            if(!in_array($com_info['type_id'],$product_cate)){
            	error_alert('商品类型不符合该优惠券');
              	return;
            }
        ...
        // dump($coupons);die;
        if($coupons){
        	$coupons_details = M('coupons')->where("id = $coupons")->find();
        	if($coupons_details['need'] <= $allTotalPrice){
        		$allTotalPrice-=$coupons_details['jian'];
		        M('coupons')->where("id = $coupons")->setField('status',2);
		        M('coupons')->where("id = $coupons")->setField('use_time',time());
        	}
        }
        $orderids = array();
        ...
        foreach ($sellerList as $key => $value) {
           ...
           foreach ($value['com_ids'] as $k => $v) {
                ...
                $tt=$total_price-$coupons_details['jian'];
                if($tt<0){
                	$tt=0;
                }
				//赠送积分
